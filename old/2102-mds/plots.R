...
for(patient in patients){
  ...
  g = ggplot(df,aes(x = MDS_D1, y = MDS_D2, colour = patient, shape = cluster, label = sample),alpha = 0.5) +
  ...
  ggsave(plot = g, paste0(file, ".png"), device = "png")
  p = plotly::ggplotly(g)
  htmlwidgets::saveWidget(p, paste0(file, ".html"))
  
  
  
  
  # gclust.dat <- t.mds[2:3]
  # mBIC = mclustBIC(gclust.dat)
  # cat(patient,file = "all.models.txt",append = T,sep = "")
  # print(paste("####################", patient, " below ####################"))
  # print(mBIC)
