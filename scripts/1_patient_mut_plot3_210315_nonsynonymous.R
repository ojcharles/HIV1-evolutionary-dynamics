# March 2021 - plots for three patients Steve wants for the preprint
# variant dynamics
# resistance mutations
# timepoint for VL
# nonsynonymous mutations in background

# remember steve this script has more filters on frequencies than the table script!

### options
analysis_outdir = "analysis/1_patientvcf2plot/"
indir = "data/vcf-mar21-trust"
patients = c("15664", "16207","22763")
background = "nonsynonymous"
###
...
for(i in iter){
  ...
  dat.plot = dat.all[dat.all$consequence == background,c(1:4, 7,6)]
  ...
  g = ggplot(data = dat.plot, aes(x = months, y = freq)) +
    ...
    scale_color_manual(values=cols, name = "Major DRMs")
  
  # headers - automatic from steves notes
  dat.header = data.frame(patient = c("22828", "22763", "16207","15664","29219","31254"), header = c("NONSYN - Patient 22828, Female, 28yrs",
                                                                                                     "NONSYN - Patient 22763, Female, 42yrs",
                                                                                                     "NONSYN - Patient 16207, Female, 35yrs",
                                                                                                     "NONSYN - Patient 15664, Male, 51yrs",
                                                                                                     "NONSYN - Patient 29219, something something",
                                                                                                     "NONSYN - Patient 31254, something something"))
