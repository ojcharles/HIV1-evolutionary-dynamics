# March 2021 - this generates the d2 table, where each column is a patient, by mutation and value is frequancy
...
files = list.files("data/vcf-csv1/",full.names = T) # vcf-csv1 if the respository that steve agreed is the correct vcf files
...
df$V1 <- NULL
df$V2 <- NULL
write.table(df, "analysis/2103-patientvcf2data/d2a.tsv",row.names = F,sep = "\t")
